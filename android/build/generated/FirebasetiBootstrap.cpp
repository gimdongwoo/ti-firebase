...
#define TAG "com.boxoutthinkers.ti.firebase"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Firebaseti_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Firebaseti.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::FirebasetiBindings::lookupGeneratedInit(
...
static void Firebaseti_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Firebaseti_getBinding)->GetFunction());
}

static void Firebaseti_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::FirebasetiBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry FirebasetiBinding = {
	"com.boxoutthinkers.ti.firebase",
	Firebaseti_init,
	Firebaseti_dispose
};
...
Java_com_boxoutthinkers_ti_firebase_FirebasetiBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.boxoutthinkers.ti.firebase", &FirebasetiBinding);
	titanium::KrollBindings::addExternalLookup(&(::FirebasetiBindings::lookupGeneratedInit));
}
